...
void RaytracerRenderer::visitLeafNode(LeafNode *leafNode) {
    ...
    if (hit) {
        if (newTime < hitRecordWithMinTime.t) {
            ...
            HitRecord newHitRecord = {newTime,intersectionPoint,normal,leafNode->getMaterial(),leafNode->getTexture(),textureCoordinates};
...
bool RaytracerRenderer::hitHappened() {
    return !(hitRecordWithMinTime.t == INFINITY);
}
